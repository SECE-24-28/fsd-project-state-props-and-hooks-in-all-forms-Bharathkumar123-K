              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      {['Customer', 'Service', 'Pet', 'Date & Time', 'Price', 'Status', 'Actions'].map(h => (
                  ...
                  <tbody>
                    {appointments.map(apt => (
                      <tr key={apt._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 font-semibold">{apt.totalPrice ? `$${apt.totalPrice}` : '—'}</td>
        ...
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              {[['Customer', selected.customer?.name], ['Service', selected.service?.name], ['Date', new Date(selected.date).toLocaleDateString()], ['Time', selected.timeSlot], ['Pet', `${selected.petName || '—'} (${selected.petType || ''})`], ['Price', `$${selected.totalPrice || 0}`]].map(([k, v]) => (
                <div key={k} className="bg-gray-50 rounded-xl p-3"><p className="text-xs text-gray-400">{k}</p><p className="font-semibold">{v}</p></div>
              ))}
            </div>
            {selected.notes && <div className="bg-blue-50 rounded-xl p-3 text-sm text-blue-800"><strong>Notes:</strong> {selected.notes}</div>}
