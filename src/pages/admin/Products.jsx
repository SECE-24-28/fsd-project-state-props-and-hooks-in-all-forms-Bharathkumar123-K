...
import { FiSearch, FiEdit, FiTrash2, FiPlus, FiAlertTriangle } from 'react-icons/fi'
...
export default function AdminProducts() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`/products/${id}`); toast.success('Removed'); fetchProducts() }
    ...
    <DashboardLayout role="admin">
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={openAdd} className="btn-primary flex items-center gap-2 text-sm py-2"><FiPlus /> Add Product</button>
        </div>

        <div className="relative max-w-sm">
          <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input className="input pl-9 text-sm" placeholder="Search products..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} />
        </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>{['Product', 'Category', 'Price', 'Sale', 'Stock', 'Actions'].map(h => (
                  ...
                  <tbody>
                    {products.map(p => (
                      <tr key={p._id} className="border-b border-gray-50 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-red-600">{p.salePrice ? formatPrice(p.salePrice) : '—'}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            ...
                            <button onClick={() => handleDelete(p._id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg"><FiTrash2 /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Product' : 'Add Product'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="btn-primary flex-1">{editId ? 'Update' : 'Add Product'}</button>
          </div>
        </form>
      </Modal>
    </DashboardLayout>
